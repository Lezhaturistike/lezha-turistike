...
console.log(`\n📊 AUDIT DESTINACIONE — CMS: ${docs.length}`)
console.log(`🧩 Jashtë katalogut 26: ${extras.length}`)
extras.forEach(d=>console.log(`   • ${d.titulli} [${d._id}]`))
console.log(`\n📍 Pa lokacion (shfaqen në katalog, përjashtohen vetëm nga routing): ${missingLocation.length}`)
...
const catalogDocs=docs.filter(d=>expected.has(d.titulli))
const mapReady=catalogDocs.filter(d=>d.hasLocation).length
console.log(`\n✅ Gati për hartë (me lokacion): ${mapReady}/${catalogDocs.length}`)
console.log(`ℹ️ Pa lokacion: ${catalogDocs.length-mapReady} — këto mbeten në katalog dhe aktivizohen automatikisht në Planner sapo të marrin lokacion në Sanity.`)
console.log(`✅ Me plannerTags: ${catalogDocs.length-missingTags.length}/${catalogDocs.length}`)
