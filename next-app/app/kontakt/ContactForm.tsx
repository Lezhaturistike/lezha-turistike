...
function ArrowUpRight() {
  return <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M7 17 17 7"/><path d="M8 7h9v9"/></svg>;
}
...
export default function ContactForm({locale = "sq"}: {locale?: Locale}) {
  ...
  return (
    <form id="contact-form" className="contact-form-card" onSubmit={submit}>
      ...
      <div className="form-actions"><button className="contact-submit" type="submit" disabled={status==="sending"}><span>{status==="sending"?t.sending:t.send}</span><span className="submit-arrow" aria-hidden="true"><ArrowUpRight/></span></button><p className="contact-form-note">{t.note}</p></div>
