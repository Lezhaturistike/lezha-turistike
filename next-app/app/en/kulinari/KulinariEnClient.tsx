...
export default function KulinariEnClient({kulinari}:{kulinari:Item[]}){
 ...
 {selected&&<div className="dialog-backdrop" onClick={close}><div className="place-dialog" role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()}><button className="dialog-close" onClick={close} aria-label="Close">×</button>{gallery[photo]?<img src={gallery[photo]} alt={selected.titulli}/>:<div className="culinary-image-placeholder dialog-placeholder"><span>LEZHË</span><small>Photo coming soon</small></div>}<div className="dialog-content"><span className="overline">{labels[selected.kategoria||""]||"CUISINE"}</span><h2>{selected.titulli}</h2><p>{selected.pershkrimi}</p><div className="culinary-details">{selected.adresa&&<div><b>Address</b><span>{selected.adresa}</span></div>}{selected.orari&&<div><b>Opening hours</b><span>{selected.orari}</span></div>}{selected.telefoni&&<div><b>Phone</b><a href={`tel:${selected.telefoni}`}>{selected.telefoni}</a></div>}</div>{gallery.length>1&&<div className="dialog-gallery">{gallery.map((p,i)=><button key={p+i} className={`gallery-thumb${photo===i?" selected":""}`} onClick={()=>setPhoto(i)}><img src={p} alt=""/></button>)}</div>}<div className="culinary-actions">{selected.harta&&<a className="button dark" href={selected.harta} target="_blank" rel="noopener noreferrer">Google Maps <span className="arrow-icon" aria-hidden="true">↗</span></a>}{selected.menuja&&<a className="button outline" href={selected.menuja} target="_blank" rel="noopener noreferrer">View menu <span className="arrow-icon" aria-hidden="true">↗</span></a>}{selected.rezervimi&&<a className="button primary" href={selected.rezervimi} target="_blank" rel="noopener noreferrer">Book <span className="arrow-icon" aria-hidden="true">↗</span></a>}{selected.website&&<a href={selected.website} target="_blank" rel="noopener noreferrer">Website <span className="arrow-icon" aria-hidden="true">↗</span></a>}{selected.social&&<a href={selected.social} target="_blank" rel="noopener noreferrer">Social media <span className="arrow-icon" aria-hidden="true">↗</span></a>}{selected.burimi&&<a href={selected.burimi} target="_blank" rel="noopener noreferrer">More information <span className="arrow-icon" aria-hidden="true">↗</span></a>}</div></div></div></div>}<style jsx>{`.culinary-image-placeholder{width:100%;height:100%;min-height:220px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;background:linear-gradient(145deg,rgba(36,95,88,.1),rgba(23,61,57,.035));color:#173d39}.culinary-image-placeholder span{font-weight:900;letter-spacing:.18em;font-size:.8rem}.culinary-image-placeholder small{font-size:.68rem;opacity:.55}:global(html[data-theme="dark"]) .culinary-image-placeholder{color:#f7f3ec;background:rgba(255,255,255,.035)}`}</style></>
}