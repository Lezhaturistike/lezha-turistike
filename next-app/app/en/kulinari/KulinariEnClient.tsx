...
export default function KulinariEnClient({kulinari}:{kulinari:Item[]}){
 ...
 <main id="home"><section id="kulinari" className="section destinations"><div className="section-top"><div><span className="overline">CUISINE</span><h1>Discover the flavours of Lezhë.</h1></div><p>Explore restaurants, farm stays, wineries and local produce.</p></div><div className="places">{items.map((x,i)=><article key={x._id} className={`place${x.featured||i===0?" big":""}`}><div className="place-image">{x.fotoUrl?<img src={x.fotoUrl} alt={x.titulli} loading="lazy"/>:<div className="culinary-image-placeholder"><span>LEZHË</span><small>Photo coming soon</small></div>}</div><div className="place-body"><span className="tag">{labels[x.kategoria||""]||"CUISINE"}</span><h3>{x.titulli}</h3><div className="culinary-rating-price"><div className="visitor-review"><div className="visitor-stars">{[1,2,3,4,5].map(n=><button key={n} type="button" onClick={()=>vote(x,n)} aria-label={`${n} stars`} className={n<=Math.round(x.rating||0)?"active":""}>★</button>)}</div><small>{x.reviewCount?`${(x.rating||0).toFixed(1)} · ${x.reviewCount} reviews`:"Leave a rating"}</small></div>{x.cmimiNga!=null&&<span className="culinary-price"><small>from</small><b>{currency[x.monedha||"EUR"]||x.monedha||"€"}{x.cmimiNga}</b>{x.cmimiPer&&<small>/ {per[x.cmimiPer]||x.cmimiPer}</small>}</span>}</div><p>{x.pershkrimi}</p>{(x.adresa||x.orari||x.telefoni)&&<div className="culinary-card-meta">{x.adresa&&<span className="culinary-meta-row"><i className="culinary-meta-icon pin" aria-hidden="true"/><em>{x.adresa}</em></span>}{x.orari&&<span className="culinary-meta-row"><i className="culinary-meta-icon clock" aria-hidden="true"/><em>{x.orari}</em></span>}{x.telefoni&&<a className="culinary-meta-row" href={`tel:${x.telefoni}`}><i className="culinary-meta-icon phone" aria-hidden="true"/><em>{x.telefoni}</em></a>}</div>}<button className="place-link-button" type="button" onClick={()=>open(x)}>View place <span className="arrow-icon" aria-hidden="true">↗</span></button></div></article>)}</div></section></main>
 <BackToTop locale="en"/>
 <SiteFooter locale="en"/>
 {selected&&<div className="dialog-backdrop" onClick={close}><div className="place-dialog" role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()}><button className="dialog-close" onClick={close} aria-label="Close">×</button>{gallery[photo]?<img src={gallery[photo]} alt={selected.titulli}/>:<div className="culinary-image-placeholder dialog-placeholder"><span>LEZHË</span><small>Photo coming soon</small></div>}<div className="dialog-content"><span className="overline">{labels[selected.kategoria||""]||"CUISINE"}</span><h2>{selected.titulli}</h2><p>{selected.pershkrimi}</p><div className="culinary-details">{selected.adresa&&<div><b>Address</b><span>{selected.adresa}</span></div>}{selected.orari&&<div><b>Opening hours</b><span>{selected.orari}</span></div>}{selected.telefoni&&<div><b>Phone</b><a href={`tel:${selected.telefoni}`}>{selected.telefoni}</a></div>}</div>{gallery.length>1&&<div className="dialog-gallery">{gallery.map((p,i)=><button key={p+i} className={`gallery-thumb${photo===i?" selected":""}`} onClick={()=>setPhoto(i)}><img src={p} alt=""/></button>)}</div>}<div className="culinary-actions"><Link className="button dark" href={`/en/planifiko?destinacion=${encodeURIComponent(selected.titulli)}`}>Plan with this place <span className="arrow-icon" aria-hidden="true">→</span></Link>{selected.harta&&<a className="button dark" href={selected.harta} target="_blank" rel="noopener noreferrer">Google Maps <span className="arrow-icon" aria-hidden="true">↗</span></a>}{selected.menuja&&<a className="button outline" href={selected.menuja} target="_blank" rel="noopener noreferrer">View menu <span className="arrow-icon" aria-hidden="true">↗</span></a>}{selected.rezervimi&&<a className="button primary" href={selected.rezervimi} target="_blank" rel="noopener noreferrer">Book <span className="arrow-icon" aria-hidden="true">↗</span></a>}{selected.website&&<a href={selected.website} target="_blank" rel="noopener noreferrer">Website <span className="arrow-icon" aria-hidden="true">↗</span></a>}{selected.social&&<a href={selected.social} target="_blank" rel="noopener noreferrer">Social media <span className="arrow-icon" aria-hidden="true">↗</span></a>}{selected.burimi&&<a href={selected.burimi} target="_blank" rel="noopener noreferrer">More information <span className="arrow-icon" aria-hidden="true">↗</span></a>}</div></div></div></div>}<style jsx>{`.culinary-image-placeholder{width:100%;height:100%;min-height:220px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;background:linear-gradient(145deg,rgba(36,95,88,.1),rgba(23,61,57,.035));color:#173d39}.culinary-image-placeholder span{font-weight:900;letter-spacing:.18em;font-size:.8rem}.culinary-image-placeholder small{font-size:.68rem;opacity:.55}:global(html[data-theme="dark"]) .culinary-image-placeholder{color:#f7f3ec;background:rgba(255,255,255,.035)}`}</style></>
}