...
export default function KulinariEnClient({kulinari}:{kulinari:Item[]}){
 ...
 {selected&&<div className="dialog-backdrop" onClick={close}><div className="place-dialog" role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()}><button className="dialog-close" onClick={close} aria-label="Close">×</button>{gallery[photo]&&<img src={gallery[photo]} alt={selected.titulli}/>}<div className="dialog-content"><span className="overline">{labels[selected.kategoria||""]||"CUISINE"}</span><h2>{selected.titulli}</h2><p>{selected.pershkrimi}</p><div className="culinary-details">{selected.adresa&&<div><b>Address</b><span>{selected.adresa}</span></div>}{selected.orari&&<div><b>Opening hours</b><span>{selected.orari}</span></div>}{selected.telefoni&&<div><b>Phone</b><a href={`tel:${selected.telefoni}`}>{selected.telefoni}</a></div>}</div>{gallery.length>1&&<div className="dialog-gallery">{gallery.map((p,i)=><button key={p+i} className={`gallery-thumb${photo===i?" selected":""}`} onClick={()=>setPhoto(i)}><img src={p} alt=""/></button>)}</div>}<div className="culinary-actions">{selected.harta&&<a className="button dark" href={selected.harta} target="_blank" rel="noopener noreferrer">Google Maps ↗</a>}{selected.menuja&&<a className="button outline" href={selected.menuja} target="_blank" rel="noopener noreferrer">View menu ↗</a>}{selected.rezervimi&&<a className="button primary" href={selected.rezervimi} target="_blank" rel="noopener noreferrer">Book ↗</a>}{selected.website&&<a href={selected.website} target="_blank" rel="noopener noreferrer">Website ↗</a>}{selected.social&&<a href={selected.social} target="_blank" rel="noopener noreferrer">Social media ↗</a>}{selected.burimi&&<a href={selected.burimi} target="_blank" rel="noopener noreferrer">More information ↗</a>}</div></div></div></div>}</>
}