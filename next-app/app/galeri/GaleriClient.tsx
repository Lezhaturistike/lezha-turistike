...
export default function GaleriClient({ destinacionet }: { destinacionet: Destinacion[] }) {
  const galleries = destinacionet.reduce((result, destinacion) => {
    ...
    const urls = destinacion.galeriaUrls && destinacion.galeriaUrls.length > 0
      ? destinacion.galeriaUrls
      : destinacion.fotoUrl ? [destinacion.fotoUrl] : [];
      ...
      <main id="home">
        <section id="galeri" className="section gallery-section">
          <div className="section-top">
            ...
            <p>Eksploro fotografitë e destinacioneve të Lezhës. Përzgjidh një fotografi për ta parë në madhësi të plotë.</p>
          </div>
          ...
          <div id="gallery-filters" className="filters" role="group" aria-label="Fotografi sipas vendit">
            ...
            {galleryEntries.map(([id, gallery]) => (
              <button key={id} type="button" className={`filter ${activeGallery === id ? "active" : ""}`} aria-pressed={activeGallery === id} onClick={() => setActiveGallery(id)}>{gallery.title}</button>
            ))}
          </div>

          <div id="gallery-grid" className="gallery-grid">
            {visibleGalleries.flatMap(([id, gallery]) => gallery.photos.map((photo, index) => (
              <button key={`${id}-${photo.src}-${index}`} type="button" className="gallery-photo" aria-label={`Hap fotografinë ${photo.alt || gallery.title}`} onClick={() => setSelected({ id, index })}>
