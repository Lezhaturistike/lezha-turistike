...
function getGalleryId(titulli: string): GalleryId | null {
  ...
  if (titulli === "Kune–Vain–Tale" || titulli === "Kune-Vain-Tale") return "kunevain";
  return null;
}
...
function fallbackSource(id: GalleryId) {
  if (id === "skenderbeu") return "https://lezha.gov.al/resurset-turistike/";
  ...
  return <>
    ...
    <main id="home">
      <section id="galeri" className="section gallery-section">
        <div className="section-top"><div><span className="overline">FOTOGRAFI NGA LEZHA</span><h1>Shiko vendet nga afër.</h1></div><p>Pamje nga Kalaja, Akrolisi, Vendvarrimi i Skënderbeut dhe Kune–Vaini. Përzgjidh një fotografi për ta parë më të madhe.</p></div>
    ...
    <footer className="site-footer" aria-label="Fundi i faqes">
      <div className="footer-main"><Link href="/" className="logo" aria-label="Lezha Turistike, faqja kryesore"><span className="logo-mark">L<span>✦</span></span><span>LEZHA<br/><b>TURISTIKE</b></span></Link><p>Një qytet për t’u zbuluar.<br/>Histori, natyrë dhe trashëgimi kulturore, të lidhura përmes hartave dhe rrëfimeve.</p><span className="footer-location">LEZHË · SHQIPËRI</span></div>
      ...
      <div className="footer-contact"><h2>Le të lidhemi</h2><p>Për informacion dhe bashkëpunime.</p><a className="footer-email" href="mailto:lezhaturistike@gmail.com">lezhaturistike@gmail.com{" "}<span aria-hidden="true"><span className="arrow-icon" aria-hidden="true">↗</span></span></a><p>Rruga e Kalasë<br/>Lezhë, Shqipëri</p><Link className="footer-contact-link" href="/kontakt">Na kontakto{" "}<span aria-hidden="true"><span className="arrow-icon" aria-hidden="true">↗</span></span></Link></div>
      <div className="footer-bottom"><span>© 2026 Lezha Turistike.</span><span>Njih historinë. Eksploro natyrën. Zbulo Lezhën.</span></div>
    </footer>
    {selected && selectedGallery && selectedPhoto && <div className="dialog-backdrop" onClick={closeDialog}><div className="place-dialog" role="dialog" aria-modal="true" aria-labelledby="dialog-title" onClick={(event)=>event.stopPropagation()}><button className="dialog-close" type="button" aria-label="Mbyll" onClick={closeDialog}>×</button><img src={selectedPhoto.src} alt={selectedPhoto.alt}/><div className="dialog-content"><span className="overline">{selectedGallery.tag}</span><h2 id="dialog-title">{selectedGallery.title}</h2><p>{selectedGallery.text}</p><div className="dialog-gallery" aria-label="Fotografi të vendit">{selectedGallery.photos.map((photo,index)=><button key={`${photo.src}-${index}`} type="button" className={`gallery-thumb ${selected.index===index?"selected":""}`} aria-label={`Shfaq fotografinë ${index+1}: ${photo.alt}`} onClick={()=>setSelected({id:selected.id,index})}><img src={photo.src} alt=""/></button>)}</div><div className="dialog-actions"><a className="button dark" href={selectedGallery.harta || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${selectedGallery.title}, Lezhë, Albania`)}`} target="_blank" rel="noopener noreferrer">Shiko vendndodhjen{" "}<span className="arrow-icon" aria-hidden="true">↗</span></a><a href={selectedGallery.source} target="_blank" rel="noopener noreferrer">Më shumë informacion{" "}<span className="arrow-icon" aria-hidden="true">↗</span></a></div></div></div></div>}
  </>;
}
