...
export default function PlannerV2Client({cmsPlaces=[],locale='sq'}:{cmsPlaces:PlannerPlace[];locale?:'sq'|'en'}){
 ...
 return <div className="plannerV2"><section className="plannerStage">
  ...
  <button className="mobileFloatToggle plannerToggle" onClick={()=>{setMobileFloat(mobileFloat==='planner'?null:'planner');setSearchOpen(false)}}>{en?'Plan your visit':'Planifiko vizitën'} <span>⌄</span></button><div className={'plannerPanel '+(mobileFloat==='planner'?'mobileOpen':'')}>
   ...
   <div className="step"><span className="stepNo">1</span><div><b>{en?'Choose interests':'Zgjidh interesat'}</b><div className="pills">{interests.map(x=><button key={x} className={interest===x?'selected':''} onClick={()=>setInterest(x)}>{x}</button>)}</div></div></div>
   <div className="step"><span className="stepNo">2</span><div><b>{en?'Available time':'Koha në dispozicion'}</b><div className="pills">{times.map(x=><button key={x} className={time===x?'selected':''} onClick={()=>setTime(x)}>{x}</button>)}</div></div></div>
   <div className="step"><span className="stepNo">3</span><div><b>{en?'Getting around':'Mënyra e lëvizjes'}</b><div className="transport">{transports.map(x=><button key={x} className={transport===x?'selected':''} onClick={()=>setTransport(x)}><span>{x==='Makinë'?'🚗':x==='Në këmbë'?'🚶':'🚲'}</span>{x}</button>)}</div></div></div>
 ...
 {qr&&<div className="qrModal" onClick={()=>setQr(false)}><div onClick={e=>e.stopPropagation()}><button className="qrClose" onClick={()=>setQr(false)}>×</button><small>{en?'MY TOUR / QR':'TURI IM / QR'}</small><h3>{en?'Scan and continue on your phone':'Skano dhe vazhdo në telefon'}</h3><img src={qrUrl} alt="QR Code"/><p>{time} · {interest}<br/>{stops.length} ndalesa · {transport}</p></div></div>}
 </div>
}