...
export default function PlannerV2Client({cmsPlaces=[],locale='sq'}:{cmsPlaces:PlannerPlace[];locale?:'sq'|'en'}){
 ...
 const[stops,setStops]=useState<PlannerPlace[]>([]),[active,setActive]=useState(0),[query,setQuery]=useState(''),[searchOpen,setSearchOpen]=useState(false),[qr,setQr]=useState(false),[mobileFloat,setMobileFloat]=useState<'themes'|'planner'|null>(null),[printFitSignal,setPrintFitSignal]=useState(0);
 ...
 return <div className="plannerV2"><div className="printTitle"><b>{en?'MY ITINERARY IN LEZHA':'ITINERARI IM NË LEZHË'}</b><span>{stops.length} {en?'stops':'ndalesa'} · {en?(transportEN[transport]||transport):transport}</span></div><section className="plannerStage">
  ...
  <button className="mobileFloatToggle plannerToggle" onClick={()=>{setMobileFloat(mobileFloat==='planner'?null:'planner');setSearchOpen(false)}}>{en?'Plan your visit':'Planifiko vizitën'} <span>⌄</span></button><div className={'plannerPanel '+(mobileFloat==='planner'?'mobileOpen':'')}>
   ...
   <div className="plannerButtons"><button className="generateV2" onClick={generate}>{en?'Generate itinerary':'Gjenero itinerarin'} <span>→</span></button><div className="plannerMiniActions"><button onClick={()=>setSearchOpen(true)}>＋ {en?'Add':'Shto'}</button><button onClick={()=>{setStops([]);setActive(0)}} disabled={!stops.length}>{en?'Clear itinerary':'Pastro itinerarin'}</button></div></div>
  </div>

  <div className="leftBottomCluster"><div className="summaryCard"><small>{en?'SUGGESTED ITINERARY':'ITINERARI I SUGJERUAR'}</small><div><span>⌖</span><b>{stops.length||'—'} {en?'stops':'ndalesa'}</b></div><div><span>↝</span><b>{stops.length?distance.toFixed(1):'—'} km</b></div><div><span>◷</span><b>{stops.length?Math.max(20,travelMinutes)+' min':'—'}</b></div></div>{stops.length>0&&<div className="selectedCards">{stops.map((p,i)=><button key={p.id} className={active===i?'active':''} onClick={()=>setActive(i)}>{p.image?<img src={p.image} alt=""/>:<span className="noPhoto">◎</span>}<span className="selectedText"><small>{en?'DESTINATION':'DESTINACIONI'} {i+1}</small><b>{p.name}</b><em>{p.desc||p.kind||'Ndalesë në itinerarin tënd.'}</em></span>{i<stops.length-1&&<strong className="nextArrow">→</strong>}</button>)}</div>}</div>
 </section>

 {stops.length>0&&<div className="mapActions"><button onClick={share} className="takeAction"><span className="actionIcon">↗</span><b>{actionLabels.share}</b><small>{actionLabels.shareSub}</small></button><button onClick={()=>setQr(true)} className="takeAction"><span className="actionIcon">▦</span><b>{actionLabels.qr}</b><small>{actionLabels.qrSub}</small></button><a href={googleMapsUrl} target="_blank" rel="noreferrer" className="takeAction"><span className="actionIcon mapActionIcon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="m3 6 5-2 8 3 5-2v13l-5 2-8-3-5 2Z"/><path d="M8 4v13M16 7v13"/></svg></span><b>{actionLabels.map}</b><small>{actionLabels.mapSub}</small></a></div>}
