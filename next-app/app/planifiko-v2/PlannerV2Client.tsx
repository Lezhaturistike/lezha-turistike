...
export default function PlannerV2Client({cmsPlaces=[]}:{cmsPlaces:PlannerPlace[]}){
 ...
 return <div className="plannerV2"><section className="plannerStage">
  ...
  <div className="plannerPanel">
   <div className="panelHead"><span className="pin mapLogo" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="m3 6 5-2 8 3 5-2v13l-5 2-8-3-5 2Z"/><path d="M8 4v13M16 7v13"/></svg></span><div><small>GUIDA JOTE DIGJITALE</small><h1>Planifiko vizitën</h1></div></div>
   ...
   <div className="plannerButtons"><button className="generateV2" onClick={generate}>Gjenero itinerarin <span>→</span></button><div className="plannerMiniActions"><button onClick={()=>setSearchOpen(true)}>＋ Shto</button><button onClick={()=>{setStops([]);setActive(0)}} disabled={!stops.length}>Pastro itinerarin</button></div></div>
  </div>
 ...
 {stops.length>0&&<section className="routeStrip"><div className="routeTitle"><div><span>ITINERARI YT</span><h2>{time} në Lezhë</h2></div><div className="routeActions"><button onClick={share} className="takeAction"><span className="actionIcon">↗</span><b>Ndaje</b><small>Dërgo itinerarin</small></button><button onClick={()=>setQr(true)} className="takeAction"><span className="actionIcon">▦</span><b>QR Code</b><small>Hape në telefon</small></button><a href={googleMapsUrl} target="_blank" rel="noreferrer" className="takeAction"><span className="actionIcon mapActionIcon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="m3 6 5-2 8 3 5-2v13l-5 2-8-3-5 2Z"/><path d="M8 4v13M16 7v13"/></svg></span><b>Hap hartën</b><small>Navigim i plotë</small></a></div></div><div className="stopStrip">{stops.map((p,i)=><button key={p.id} className={active===i?'active':''} onClick={()=>setActive(i)}><span>{i+1}</span><div><b>{p.name}</b><small>{p.kind||'Destinacion'}</small></div></button>)}</div></section>}
